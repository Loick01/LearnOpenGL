...
unsigned int loadTexture(char const *path)
{
    ...
    if (data)
    {
        ...
        if (nrComponents == 1)
            ...
            format = GL_RGB;
            // Using GL_SRGB/GL_SRGB_ALPHA will load textures in linear space
            // format = GL_SRGB; // Should only be used for 3rd parameter in glTexImage2D, not for the 7th
        else if (nrComponents == 4)
            format = GL_RGBA;
            // format = GL_SRGB_ALPHA; // Should only be used for 3rd parameter in glTexImage2D, not for the 7th
...
int main()
{
    ...
    // glEnable(GL_FRAMEBUFFER_SRGB); // Enable gamma correction with a gamma of 2.2 (standard for most monitors) 
    // Enabling it here will also affect glClearColor
